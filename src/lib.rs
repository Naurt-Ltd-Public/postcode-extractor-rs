...
pub use country::{
	Country, FIVE_DIGIT_ADDITIONAL_NATIONS, FIVE_DIGIT_NATIONS,
	FIVE_DIGIT_WITH_SPACE_NATIONS, FOUR_DIGIT_NATIONS, SIX_DIGIT_NATIONS,
	UNIQUE_COUNTRIES,
};

static TEMPLATES_DIR: Dir =
	include_dir!("$CARGO_MANIFEST_DIR/postalcode-extractor/regex");
...
pub fn evaluate_all_countries(
	...
	for country in UNIQUE_COUNTRIES.iter().cloned() {
		if let Ok(Some(pc)) =
			evaluate_single_country(haystack, country.clone(), check_position)
		{
	...
	if let Ok(Some(pc)) =
		evaluate_single_country(haystack, Country::CZ, check_position)
	{
	...
	if let Ok(Some(pc)) =
		evaluate_single_country(haystack, Country::GR, check_position)
	{
	...
	if let Ok(Some(pc)) = evaluate_single_country(
		haystack,
	...
	if let Ok(Some(pc)) =
		evaluate_single_country(haystack, Country::US, check_position)
	{
	...
	if check_position {
		if let Ok(Some(pc)) =
			evaluate_single_country(haystack, Country::TW, check_position)
		{
			return Ok(Some(PostcodeWrapper {
				country: Country::TW,
				postcode: pc,
	...
	// Cyprus special case
	if let Ok(Some(pc)) =
		evaluate_single_country(haystack, Country::CY, check_position)
	{
	...
	if let Ok(Some(pc)) =
		evaluate_single_country(haystack, Country::LU, check_position)
	{
