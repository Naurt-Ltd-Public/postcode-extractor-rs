use postcode_extractor::{
	...
	FIVE_DIGIT_WITH_SPACE_NATIONS, FOUR_DIGIT_NATIONS, PostcodeWrapper,
	SIX_DIGIT_NATIONS, evaluate_all_countries,
};
	...
	for doc in WalkDir::new("postalcode-extractor/tests") {
		...
		if path.is_file() {
			...
			for case in &test_holder.tests.position_check.matches {
				let message =
					format!("Evaluating {} in {:?}", case.address, path);
				...
				if let Some(add) = &case.postcode_additional {
					assert_eq!(
						add,
						holder.additional.as_ref().unwrap(),
						"{}",
						message
					);
				}
			...
			for case in &test_holder.tests.no_position_check.matches {
				let message =
					format!("Evaluating {} in {:?}", case.original, path);
...
fn verify_country(
	wrapper: &PostcodeWrapper,
	test_holder: &TestHolder,
	debug_msg: &str,
) {
	if SIX_DIGIT_NATIONS.contains(&test_holder.country) {
		if wrapper.country != Country::Unknown6Digit
			&& !(wrapper.country == Country::TW
				&& test_holder.country == Country::TW)
		{
			panic!(
				"Expected {:?}; got {:?} {}",
		...
		if wrapper.country != Country::Unknown4Digit
			&& !(wrapper.country == Country::CY
				&& test_holder.country == Country::CY)
			&& !(wrapper.country == Country::LU
				&& test_holder.country == Country::LU)
		{
			panic!(
				"Expected {:?}; got {:?} {}",
				test_holder.country, wrapper.country, debug_msg
		...
		if wrapper.country != Country::Unknown5DigitSpace
			&& !(wrapper.country == Country::CZ
				&& test_holder.country == Country::CZ)
			&& !(wrapper.country == Country::GR
				&& test_holder.country == Country::GR)
		{
			panic!(
				"Expected {:?}; got {:?} {}",
				test_holder.country, wrapper.country, debug_msg
